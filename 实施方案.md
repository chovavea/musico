...
已确认：完整站点 + Docker；Python FastAPI + Vue 3；**PostgreSQL** 存榜单快照；热榜 + 官方预览；第一版只跑通 QQ 音乐和网易云。界面采用简洁的音乐应用骨架和克制的毛玻璃效果。内核无音源、按能力拆插件（Chart / Preview / Search / Media）。下载能力只留 `MediaPort`，默认关闭且无实现。
...
- [x] Docker / README / pre-commit / structlog；对照下方「完成定义」验收（本机无 Docker，镜像与 compose 待现场验收）
...
第二期再考虑 Redis 的信号：多副本、Matcher 算得很重、或需要支撑高频读。到时 Redis 只缓存「最新榜 JSON」，Postgres 仍是真相源。
...
musico/
  ...
  web/src/{pages,components,composables,stores}  # Pinia + Vue Router，不引入 Vuex / 额外 UI 库
  configs/boards.yaml
...
- 持续集成：fixture 单测 + 前端 `npm run build` + 构建应用镜像；默认不访问外网
...
5. 跨平台试听回退（任意平台都能用其他平台的官方试听，见下节设计）

## 试听回退：跨平台官方试听

目标：任意平台曲目在原平台官方试听不可用时，自动改用**其他平台的官方试听**（流式代理），不建下载任务、不写曲库。通用化：平台只要在 `plugin.toml` 声明 `search` + `preview` 能力就自动参与回退，核心不写平台分支。

### 分层（硬顺序，逐档可开关）

| 档 | 来源 | 说明 |
| --- | --- | --- |
| T0 | 本地曲库 `ready` | 现有逻辑，离线最优 |
| T1 | 原平台官方试听 | `external_id` 天然对齐，无匹配风险 |
| T2 | 其他平台官方试听 | 本期新增；需匹配校验 |
| T3 | 下载源插件 | 现有的最后一道，按优先级逐个源串行尝试，只出试听流、不建下载任务 |
| T4 | 页面内试听源 | Sonoma / flmp3 / gequbao 同属严格匹配试听站，以来源 id 区分 |
| T7 | 宽松页面试听 | T4 全部失败后才启用的低置信兜底 |

T1 先于 T2：原平台试听与榜单条目是同一 id，零匹配风险；跨平台只能靠文本匹配，错配代价高（听到 Live / 翻唱）。

### 候选排序算法（不设静态平台优先级）

`rate(origin, target)`＝该平台组合的历史可播率，取 Beta 后验 `(成功 + 1) / (总数 + 2)`；样本不足时先退化为按来源平台聚合、再退化为全局；冷启动各平台先验相同（0.5），不偏袒任何平台。

样本口径：本次出音的 target 记成功；`planner.contacted` 里被问过却没出音的 target（搜到候选但打不开、候选全被守卫拒掉、搜索返回空）逐个记失败；搜索抛错/超时的平台不记样本，避免网络抖动误伤正常平台。失败必须落库（`tier=T2 / status=error`），否则重启后 `prewarm()` 只能从成功里恢复，降权会丢失。

排序键（从主到次）：

1. 平台之间的顺序：`rate(origin, target)` → 平台 id 字典序（只为结果可复现，不是需要维护的优先级配置）
2. 同一平台内部的候选顺序：匹配分 `track_match_score`（ISRC 1.0 / 时长差 ≤2s 0.98 / ≤5s 0.94 / 仅歌名歌手 0.90）→ 搜索排名；每平台最多试 `PREVIEW_MAX_CANDIDATES` 个，官方试听质量 `medium` > `low` > 未知 仅作为记录字段

执行（同类内延迟对冲，**跨档不并发**）：按平台顺序启动第一个平台，超过对冲延迟仍未出音再并行第二个，最多两个在途；第一个通过「首字节非空且不是 HTML」检查的就是本次播放源，另一个立即取消。整个阶段受 `PREVIEW_DEADLINE_SEC` 约束，超时直接落到下载源档且**不写负缓存**（预算耗尽只说明这次没跑完，不能证明这首歌放不出来）。

### T2–T7 同类内自适应排序

类别顺序固定为 `T1 → T2 → T3 → T4 → T7`，不做跨类别竞争；只调整每一类内部的供应商顺序。T4 的 Sonoma、flmp3、gequbao 语义相同，统一记录为 `tier=T4`，供应商由 `source_platform` 区分。

每个实际来源尝试记录独立的成功/失败和首字节耗时。可播率继续使用 Beta 平滑，耗时使用按时间半衰的近期加权均值；同类来源按 `expected_cost = latency / optimistic_playability` 从小到大排序。`optimistic_playability` 带小幅 UCB 探索奖励，防止新源或已恢复来源永久排在末尾。旧样本默认每 3 天衰减一半，搜索异常、超时、无匹配和死链都作为短期失败；不做额外探测或并发抢跑。

冷启动及关闭 `PREVIEW_ADAPTIVE_ORDER` 时恢复确定顺序：T2 按平台 id、T3 按下载源 priority、T4 按 Sonoma → flmp3 → gequbao。T7 使用独立统计，避免宽松匹配结果污染严格 T4 的可播率。

缓存命中不再重复记账：正缓存命中只重放上一次的决定，不再投阳性样本、也不再落 `preview_event` 行（否则同一次上游成功会被每次点击重复计数，重启后 `prewarm()` 还会把这些行当成新的成功）。缓存的签名地址过期打不开时立即 `invalidate_positive()` 丢弃该条，后续点击直接重新搜索，不会每次都先白打一次死链。

落库不挡首字节：`preview_event` 的写入在响应开始流式输出后作为后台任务执行（仍挂在响应上以便异常可见），成功路径不再为一次 DB 往返推迟出声。

### 匹配与版本守卫

- 跨平台阈值 `PREVIEW_MATCH_MIN_SCORE` 默认 0.9（同平台不校验），ISRC 命中直接放行。
- 版本守卫：原曲标题不含版本词而候选含版本词（Live / 伴奏 / Remix / 现场）→ 直接拒绝。`normalize_text` 会把这类括号整段删掉，不能依赖它区分版本。
- 歌手：要求主歌手匹配且歌手集合有交集，不能只比 `artist_key` 取到的首位。
- 榜单链路携带时长后，匹配分能优先使用录音时长而不是只依赖文本。

### 探测、缓存与预算

- 不新增独立探测请求：直接复用 `adapters/http/preview.py` 里 `_open_audio()` 的「首字节非空且非 HTML」判断当探测。
- 跨档不并发扇出：T1 → T2 → T3 → T4 → T7 仍按类别串行。同一类内部默认延迟对冲，最多两个来源在途；`PREVIEW_HEDGE_ENABLED=false` 时退回一个来源失败后才问下一个。
- 缓存键用 `platform:external_id`（榜单没有时长，比 `track_key` 稳）：正 5–10 分钟、负 3–5 分钟；内存 LRU，多实例各存一份可接受。
- 单次点击预算 `PREVIEW_DEADLINE_SEC`（默认 60s），覆盖「搜索 + 逐个候选解析试听地址 + 打开音频读首字节」，超时落到 T3 且不写负缓存；每平台候选上限 `PREVIEW_MAX_CANDIDATES`（默认 2）。
- 出站调用不做单次限流：`PreviewPlanner` 不再给 `search()` / `preview()` 套 `asyncio.timeout`（旧的 `PREVIEW_SEARCH_TIMEOUT_SEC=3` 已删除），慢但从不出错的平台不会被误判成不可播。调用层面若需要上限，一律按 1 分钟，且只有 `PREVIEW_CALL_TIMEOUT_SEC`（默认 60）这一个旋钮：预览 HTTP 客户端 `httpx.Timeout(...)`、打开音频、T3 下载源搜索/解析共用它（`preview_call_timeout()` 未接配置时回落到 `_CALL_TIMEOUT_SEC`）。因此整条预览链路只会被两件事提前结束：整段预算 `PREVIEW_DEADLINE_SEC`，或单次调用满 `PREVIEW_CALL_TIMEOUT_SEC`。

### 数据库记录（仅排查用，前端不展示）

append-only 表 `preview_event`：`track_key / origin_platform / external_id / title / artist / tier / source_platform / source_external_id / match_score / status / error / latency_ms / created_at`。T2–T7 按每个实际来源尝试落行，`latency_ms` 是该来源的局部耗时；历史 T5/T6 在预热时映射到 T4 命名空间。

- 统计表 `preview_source_stat(origin_platform, target_platform, success, failure, rate, avg_latency_ms, updated_at)` 由事件表聚合，服务启动时用近 7 天事件预热内存。
- **前端不展示来源**：不加 `/plan` 接口、不加播放条徽标；来源只进数据库与结构化日志（`preview_source_selected`）。

### 接口与代码落点

- `ports/search.py`：`SearchPort.search(TrackQuery) -> list[TrackRef]`（`TrackQuery` = title / artist / album / duration_ms / isrc / limit），返回带时长与 ISRC 的 `TrackRef` 供打分。
- 插件新增 `search.py` + `create_search()`，`plugin.toml` 的 `capabilities` 加 `"search"`：网易用 `https://music.163.com/api/search/get`（实测可用），QQ 用 `musicu.fcg` 的 `music.search.SearchCgiService`（实测 code 0）。
- 新服务 `services/preview_plan.py`：`PreviewPlanner(track, registry, policy).iter_targets()` 是惰性异步迭代器，按平台顺序逐个产出候选（`contacted` 记录真正被问过的平台，`attempted` 供调用方决定负缓存；`searched` 是 `contacted` 的派生属性），只依赖 `PreviewPort` / `SearchPort`，不碰下载源。跨平台缓存与可播率计数在 `services/preview_telemetry.py`。
- `adapters/http/preview.py` 是显式管线：T0 → T1 → T2 → T3 → T4 → T7；`services/preview_ladder.py` 只负责各类内部的来源排序。
- 配置增加 `PREVIEW_ADAPTIVE_ORDER`、`PREVIEW_EXPLORE_WEIGHT`、`PREVIEW_DECAY_HALF_LIFE_SEC`；其余匹配、预算和缓存配置保持不变。
- 前端：`stores/player.ts` 的 `startFallback()` 不再传「跳过官方试听」的参数，改为「原平台官方已由 SDK 试过、仍允许跨平台与下载源」。

### 里程碑与验收

1. M1 管线重构：抽出 `preview_plan.py`，行为不变，现有后端用例保绿。**已完成**（`server/app/services/preview_plan.py`）。
2. M2 跨平台回退：`search` 能力 + 网易/QQ 搜索 + T2 + 版本守卫 + 负缓存 + `preview_event` 落库 + 测试。**已完成**。
3. M3 数据质量：榜单补 `duration_ms` 并透出到 API（`platform_song.duration_ms`、`payload_from_raw`）。**已完成**。
4. M4 可选：VIP cookie 扩展、Redis 缓存、设置界面开关。**未开始**。

落库与排查：`musico_library.preview_event`（每次点击一行）+ `musico_library.preview_source_stat`（启动按近 7 天事件刷新），迁移 `0008_preview_events`。来源不进 `/api` 响应，前端不展示。

验收：QQ 付费曲（如 `001fsNdn1zuZnA`）在网易侧可播时能出声；`preview_event` 能查到本次实际来源；全部候选不可用时前端显示「暂无可用试听音源」，不自动跳歌、不建下载任务。

### 已定决策

1. 平台顺序不做静态默认配置，由上面的算法决定。
2. 试听来源写数据库用于排查，**前端不展示**。
3. T3 后保留统一的 T4 页面试听档；T7 宽松匹配始终最后。
4. 跨档串行；同类内最多两个来源延迟对冲，不做全量扇出。
