...
执行（顺序逐个平台，**不并发**）：按平台顺序取第一个平台 → 搜索 → 排序候选 → 解析 preview URL → 打开音频流；只有确认放不出音才去问下一个平台，第一个通过「首字节非空且不是 HTML」检查的就是本次播放源。同一时刻最多一条出站请求，因此请求量与首字节延迟都只取决于排在前面的平台数量。成功记正缓存并抬高 `rate`，失败记负缓存并压低 `rate`；整个阶段受 `PREVIEW_DEADLINE_SEC` 约束，超时直接落到下载源档且**不写负缓存**（预算耗尽只说明这次没跑完，不能证明这首歌放不出来）。
...
- 跨平台阈值 `PREVIEW_MATCH_MIN_SCORE` 默认 0.94（同平台不校验），ISRC 命中直接放行。
- 版本守卫：原曲标题不含版本词而候选含版本词（Live / 伴奏 / Remix / 现场）→ 直接拒绝。`normalize_text` 会把这类括号整段删掉，不能依赖它区分版本。
- 歌手：要求主歌手匹配且歌手集合有交集，不能只比 `artist_key` 取到的首位。
- 前置缺口：榜单链路目前丢掉了时长（`services/catalog.py` 的 `payload_from_raw` 不透出，`plugins/*/charts.py` 的解析器也没填，而原始数据里有网易 `dt`、QQ `interval`）。补齐后匹配才能稳定在 0.94 以上。

### 探测、缓存与预算

- 不新增独立探测请求：直接复用 `adapters/http/preview.py` 里 `_open_audio()` 的「首字节非空且非 HTML」判断当探测。
- 不做并发扇出：T1 → T2 → T3 整条链路串行，T2 是「一个平台失败才问下一个」，T3 是「一个下载源失败才问下一个」（`_download_candidates()` 是按优先级排序的惰性异步生成器，只有上一个源的可播候选全部试完才会去搜索下一个源）。因此不需要为并发单独加限流；`PREVIEW_MIN_INTERVAL_SEC` 仅在将来需要跨歌曲节流时才用得上。
...
新增 append-only 表 `preview_event`：`track_key / origin_platform / external_id / title / artist / tier / source_platform / source_external_id / match_score / status / error / latency_ms / created_at`。每次试听请求结束时落行（成功与失败都写），跨平台档里每个「问过但没出音」的 target 另落一行 `tier=T2 / status=error / source_platform=<target>`，用于回答「这次到底用了哪个源、为什么没播」。
...
- `adapters/http/preview.py` 改成显式管线：T0 → T1 → T2（按 plan 顺序）→ T3（`_download_candidates()` 按优先级逐个下载源，源内候选按格式轻重排序）。
- 配置：`PREVIEW_CROSS_PLATFORM`、`PREVIEW_MATCH_MIN_SCORE`、`PREVIEW_MAX_CANDIDATES`、`PREVIEW_CALL_TIMEOUT_SEC`、`PREVIEW_DEADLINE_SEC`、`PREVIEW_NEGATIVE_TTL_SEC`、`PREVIEW_POSITIVE_TTL_SEC`。`PREVIEW_MIN_INTERVAL_SEC` 仍未使用（搜索限流留给 M4，目前靠正/负缓存抑制重复搜索）。
...
3. M3 数据质量：榜单补 `duration_ms` 并透出到 API（`platform_song.duration_ms`、`payload_from_raw`），阈值收紧到 0.94。**已完成**。
