...
from app.domain.models import BoardSpec, RawRankItem, TrackRef
...
class ChartRepository:
    ...
    async def _annotate_library(self, entries: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            library_rows = list(tracks.values())
            for item in entries:
                track = next(
                    (
                        candidate
                        for candidate in library_rows
                        if is_same_recording(_chart_item_ref(item), _library_ref(candidate))
                    ),
                    None,
                )
                if track is None:
...
def _chart_item_ref(item: dict[str, Any]) -> TrackRef:
    return TrackRef(
        platform=str(item.get("platform") or "chart"),
...
def _library_ref(row: LibraryTrackRow) -> TrackRef:
    return TrackRef(
        platform="library",
        external_id=row.id,
        title=row.title,
